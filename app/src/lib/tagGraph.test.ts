...
describe("buildTagGraph", () => {
  ...
  it("only includes tags used by an approved app", async () => {
    ...
    const used = await prisma.tag.create({ data: { id: "used", slug: "used", name: "used" } });
    const orphan = await prisma.tag.create({ data: { id: "orphan", slug: "orphan", name: "orphan" } });
  ...
  it("sums stake and counts apps per tag", async () => {
    const appA = await prisma.app.create({ data: { id: "a", slug: "a", name: "A", url: "https://example.com", status: "approved" } });
    const appB = await prisma.app.create({ data: { id: "b", slug: "b", name: "B", url: "https://example.com", status: "approved" } });
  ...
  it("computes co-occurrence weight and Jaccard similarity between two tags", async () => {
    ...
    const app1 = await prisma.app.create({ data: { id: "app1", slug: "app1", name: "App1", url: "https://example.com", status: "approved" } });
    const app2 = await prisma.app.create({ data: { id: "app2", slug: "app2", name: "App2", url: "https://example.com", status: "approved" } });
    const app3 = await prisma.app.create({ data: { id: "app3", slug: "app3", name: "App3", url: "https://example.com", status: "approved" } });
    const defi = await prisma.tag.create({ data: { id: "defi", slug: "defi", name: "defi" } });
    const nft = await prisma.tag.create({ data: { id: "nft", slug: "nft", name: "nft" } });
