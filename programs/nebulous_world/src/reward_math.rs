use crate::constants::{CONFIG_SEED, REWARD_PRECISION};
...
/// Transfer `amount` out of the single global vault (see the design note on
/// `Config`), signed by the `config` PDA via its derivation seeds
/// (`[CONFIG_SEED]`). Every payout in the program — vote-reward, tags-reward,
/// vote-principal, tag-principal — goes through this one helper, since
/// `config` is now the vault's only authority (there is no more per-app or
/// per-tag PDA signer).
///
