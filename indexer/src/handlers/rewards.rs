...
async fn positions(State(state): State<Arc<ApiState>>, Query(q): Query<PositionsQuery>) -> Result<Json<serde_json::Value>, ApiError> {
    let vote_rows: Vec<(String, f64, String, String)> = sqlx::query_as(
        r#"
        SELECT v."appId", v.amount, a.slug, a.name
    ...
    for (app_id, amount, slug, name) in vote_rows {
        vote_by_app
            .entry(app_id.clone())
            .and_modify(|v| v.amount += amount)
            .or_insert(VotePositionDto { app_id, app_slug: slug, app_name: name, amount });
    }

    let stake_rows: Vec<(String, f64, String, String, String, String, String)> = sqlx::query_as(
        r#"
        SELECT s."appTagId", s.amount, at."appId", a.slug, a.name, t.slug, t.name
        FROM "Stake" s
    ...
    for (app_tag_id, amount, app_id, app_slug, app_name, tag_slug, tag_name) in stake_rows {
        stake_by_app_tag
            .entry(app_tag_id.clone())
            .and_modify(|s| s.amount += amount)
            .or_insert(StakePositionDto { app_tag_id, app_id, app_slug, app_name, tag_slug, tag_name, amount });
    }
